...
export default class AccountsController extends Controller {
    ...
    remove(id) {
        if (AccessControl.writeGrantedAdminOrOwner(this.HttpContext, this.requiredAuthorizations, id)) {
            try {
                const postsLikesController = new PostsLikesController(this.HttpContext);
                const deletedLikes = postsLikesController.removeLikesByUserId(id);
                console.log(`Likes supprimés : ${deletedLikes}`);
    
                const postsController = new PostsController(this.HttpContext);
                const deletedPosts = postsController.removePostsByWriterId(id);
                console.log(`Posts supprimés : ${deletedPosts}`);
        
                const userDeleted = this.repository.remove(id);
                if (userDeleted) {
                    return this.HttpContext.response.ok({ message: "Compte supprimé avec succès." });
                } else {
                    return this.HttpContext.response.notFound({ message: "Utilisateur introuvable." });
                }
            } catch (error) {
                console.error("Erreur lors de la suppression :", error);
                return this.HttpContext.response.serverError({ message: "Une erreur est survenue." });
            }
        } else {
            // Si l'utilisateur n'est pas autorisé
            return this.HttpContext.response.unAuthorized({ message: "Accès non autorisé." });
        }
    }
